...
export async function getWhatsAppNumber() {
  const settings = await getSiteSettings().catch(() => null);
  return settings?.contactInfo?.whatsappNumber || env.VITE_WHATSAPP_NUMBER || '919281424494';
}

export async function getContactPhone() {
  const settings = await getSiteSettings().catch(() => null);
  return settings?.contactInfo?.mainPhone || '9281424494';
}

export const SITE_URL = cleanSiteUrl(env.VITE_SITE_URL) || PRODUCTION_SITE_URL;
export const WHATSAPP_NUMBER = env.VITE_WHATSAPP_NUMBER || '919281424494';
export const CONTACT_PHONE = '9281424494';
