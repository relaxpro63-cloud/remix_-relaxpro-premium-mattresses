...
export default function SleepScience() {
  ...
  const comp = data?.comparisonSection || {};
  const ortho = data?.orthopedicSection || {};
  const certs = data?.certifications || [];
  const ctaLink = data?.ctaLink || '/compare';
  const tabKeys: ('kerala' | 'postures' | 'certifications')[] = ['kerala', 'postures', 'certifications'];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-6 font-body">
                {(ortho?.items && ortho.items.length > 0 ? ortho.items : [
                  { title: 'Side Sleeper Comfort Zone', desc: 'Requires local indentation for shoulders and pelvis while pushing back on your ribcage. Our <strong>Amrita</strong> & <strong>Nirvana</strong> models are optimal.' },
