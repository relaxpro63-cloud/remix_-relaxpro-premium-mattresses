...
function LayerStack({ build, config }: { build: BuildState; config: BuilderConfig }) {
  ...
  return (
    <div className="relative bg-ink-900 rounded-2xl p-5 overflow-hidden">
    ...
    <div>
          ...
          return (
            <motion.button
              ...
              className={`relative p-4 rounded-xl text-center border-2 transition-all duration-200 cursor-pointer ${
                active ? 'border-brand-600 bg-brand-50 shadow-sm' : 'border-graphite-100 bg-white hover:border-graphite-200'
              }`}
            >
              <Bed className={`w-5 h-5 mx-auto mb-1.5 ${active ? 'text-brand-600' : 'text-ink-900'}`} />
              <span className="font-bold text-sm capitalize block text-ink-900">{s.name}</span>
              <span className="text-[10px] font-medium mt-1 block text-graphite-400">{s.widthInches}×{s.lengthInches}</span>
              {s.popular && <span className="text-[9px] font-bold text-brand-600 mt-1 block">Popular</span>}
              {active && <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-brand-600 flex items-center justify-center"><Check className="w-3 h-3 text-white" /></div>}
            </motion.button>
          );
        })}
        {config.customSize.enabled && (
          <motion.button
            ...
            className={`relative p-4 rounded-xl text-center border-2 border-dashed transition-all duration-200 cursor-pointer ${
              build.size.kind === 'custom' ? 'border-brand-600 bg-brand-50' : 'border-graphite-200 bg-white hover:border-graphite-300'
            }`}
          >
            <Ruler className="w-5 h-5 mx-auto mb-1.5 text-graphite-400" />
      ...
      <AnimatePresence>
        {showCustom && (
          ...
          >
            <div className="mt-4 p-4 bg-graphite-50 rounded-xl border border-graphite-100">
              <div className="grid grid-cols-2 gap-4 mb-3">
                <div>
                  <label className="text-xs font-semibold text-graphite-600 block mb-1">Length ({config.customSize.unit})</label>
                  <input type="number" value={custL} onChange={e => setCustL(Number(e.target.value))}
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-graphite-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-600/30"
                    onBlur={() => onSelect({ ...build, size: { kind: 'custom', length: custL, width: custW } })}
                  />
                </div>
                <div>
                  <label className="text-xs font-semibold text-graphite-600 block mb-1">Width ({config.customSize.unit})</label>
              ...
              {config.customSize.helper && (
                <p className="text-[11px] text-graphite-400">{config.customSize.helper}</p>
              )}
            </div>
          </motion.div>
        )}
...
function StepMaterial({ slotLabel, slot, materials, build, onSelect }: {
  ...
  return (
    <div>
      <p className="text-[11px] text-graphite-400 mb-3">Tap to add, tap again to remove. You can select multiple layers.</p>
            ...
            <div key={mat.slug}>
              <motion.button
                ...
                className={`relative w-full p-4 rounded-xl border-2 text-left transition-all duration-200 cursor-pointer ${
                  selected ? 'border-brand-600 bg-brand-50 shadow-sm' : 'border-graphite-100 bg-white hover:border-graphite-200'
                }`}
              >
                  ...
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {mat.isRecommended && <span className="text-[9px] font-bold text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded">Recommended</span>}
                      </div>
                      {mat.feelTag && <p className="text-xs text-graphite-500 mt-0.5">{mat.feelTag}</p>}
                      {mat.benefit && <p className="text-[11px] text-graphite-400 mt-0.5">{mat.benefit}</p>}
                    </div>
                  </div>
                  {(mat.density || mat.ild) && (
                    <span className="shrink-0 text-[10px] font-semibold text-graphite-400 bg-graphite-50 px-2 py-1 rounded-lg flex items-center gap-1">
                ...
                {selected && (
                  <span className="text-[10px] text-brand-600 font-semibold mt-2 block">Tap to remove</span>
                )}
              </motion.button>
              <AnimatePresence>
                        ...
                        return (
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all cursor-pointer ${
                              isActive ? 'border-brand-600 bg-brand-50 text-brand-600' : 'border-graphite-100 bg-white text-graphite-500 hover:border-graphite-200'
                            }`}
                          >
                            {t.label}
                          </button>
...
function StepCover({ fabrics, build, onSelect }: {
  ...
  return (
    <div>
      <p className="text-xs text-graphite-500 mb-4">{/* Cover helper */}</p>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-sm text-ink-900">{fab.name}</span>
                    {fab.isRecommended && <span className="text-[9px] font-bold text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded">Recommended</span>}
                  </div>
                  {fab.benefit && <p className="text-xs text-graphite-500 mt-0.5">{fab.benefit}</p>}
                  {fab.gsm && <p className="text-[10px] text-graphite-400 mt-0.5">{fab.gsm}</p>}
                </div>
              </div>
              {active && <div className="absolute top-3 right-3"><Check className="w-4 h-4 text-brand-600" /></div>}
            </motion.button>
          );
        })}
        {quiltFab && (
          <div className="pt-2 border-t border-graphite-100">
            <label className="flex items-center gap-3 p-3 rounded-xl border border-graphite-100 bg-white cursor-pointer hover:border-graphite-200 transition-colors">
              <div className={`w-5 h-5 rounded flex items-center justify-center border-2 transition-colors ${
                build.cover.quiltingSlug ? 'bg-brand-600 border-brand-600' : 'border-graphite-300'
              }`}>
              ...
              {quiltFab.quiltingMm && <span className="text-[10px] font-semibold text-graphite-400">{quiltFab.quiltingMm}</span>}
            </label>
          </div>
        )}
      </div>
...
function BuilderSummary({ build, config, price, addedToCart, onAddToCart, onWhatsApp }: {
  ...
  return (
    <div>
      <h3 className="font-bold text-base text-ink-900 tracking-tight mb-4">{config.summaryPanel.heading}</h3>

      {/* Size */}
      <div className="flex items-center gap-2 mb-4 text-sm">
        <Ruler className="w-4 h-4 text-brand-600" />
        <span className="font-semibold text-ink-900">{build.size.name || 'Custom'}</span>
        <span className="text-graphite-400">· {build.size.width}×{build.size.length}</span>
      </div>
      ...
      {/* Text receipt */}
      {isComplete && (
        <div className="mt-5 pt-4 border-t border-graphite-200/60">
          <h4 className="text-[11px] font-bold text-ink-900 uppercase tracking-wider mb-3">Your Build Specs</h4>
          <div className="space-y-3 text-xs text-graphite-600">
            <div className="flex items-start gap-2">
              <span className="font-semibold text-graphite-700 shrink-0 w-20">Size:</span>
              <span>{build.size.name || 'Custom'} ({build.size.width}×{build.size.length} in)</span>
            </div>
            {comfortMats.length > 0 && (
              <div className="flex items-start gap-2">
                <span className="font-semibold text-graphite-700 shrink-0 w-20">Comfort:</span>
                <span>{comfortMats.map(s => `${s.mat?.name || s.materialSlug} (${s.thickness}")`).join(', ')}</span>
              </div>
            )}
            {supportMats.length > 0 && (
              <div className="flex items-start gap-2">
                <span className="font-semibold text-graphite-700 shrink-0 w-20">Support:</span>
                <span>{supportMats.map(s => `${s.mat?.name || s.materialSlug} (${s.thickness}")`).join(', ')}</span>
              </div>
            )}
            <div className="flex items-start gap-2">
              <span className="font-semibold text-graphite-700 shrink-0 w-20">Cover:</span>
      ...
      {/* Feel summary */}
      {feelTags.length > 0 && (
        <p className="text-xs text-graphite-500 mt-4 leading-relaxed">
          {feelTags.join(' · ')}
        </p>
      )}
      ...
      {chips.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-3">
          {chips.map(c => (
            <span key={c} className="text-[10px] font-semibold text-graphite-500 bg-graphite-100 px-2 py-0.5 rounded-full">{c}</span>
          ))}
        </div>
      )}

      {/* Price */}
      <div className="mt-6 pt-5 border-t border-graphite-100">
      ...
      <div className="mt-6 space-y-2">
        <motion.button
          ...
          className={`w-full py-3.5 px-4 rounded-xl font-bold text-sm tracking-wide transition-all duration-200 flex items-center justify-center gap-2 ${
            isComplete ? 'bg-ink-900 text-white shadow-md cursor-pointer' : 'bg-graphite-200 text-graphite-400 cursor-not-allowed'
          }`}
          disabled={!isComplete}
        >
        ...
        <motion.button
          onClick={onWhatsApp}
          whileTap={{ scale: 0.98 }}
          className="w-full py-3 px-4 rounded-xl border-2 border-eco-500/20 text-eco-600 hover:bg-eco-50 font-semibold text-xs tracking-wide transition-all flex items-center justify-center gap-2 cursor-pointer"
        >
  ...
  if (error) return (
    <div className="min-h-[60vh] flex items-center justify-center">
      <div className="text-center max-w-md">
        ...
        <p className="text-graphite-400 text-xs mt-1">{error}</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-12 md:py-20">
      ...
      <div className="text-center max-w-2xl mx-auto mb-12">
        <span className="inline-flex items-center gap-1.5 text-[11px] tracking-[0.2em] font-semibold text-brand-600 uppercase mb-4">
          <Sparkles className="w-3.5 h-3.5" /> Direct From Kerala Factory
        </span>
        <h1 className="text-4xl md:text-5xl font-heading font-bold tracking-tight text-ink-900 leading-tight">
          {config.header.title}
        </h1>
        <p className="text-graphite-700 mt-3 text-sm md:text-base leading-relaxed max-w-lg mx-auto">
          {config.header.subtitle}
        </p>
        {config.header.trustChips && (
          <div className="flex flex-wrap justify-center gap-2 mt-4">
            {config.header.trustChips.map(c => (
              <span key={c} className="text-[10px] font-semibold text-brand-600 bg-brand-50 border border-brand-600/10 px-2.5 py-1 rounded-full">{c}</span>
            ))}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div className="lg:col-span-5 lg:sticky lg:top-24 order-1">
          <div className="bg-white rounded-3xl border border-graphite-100/80 shadow-[0_12px_40px_rgba(0,0,0,0.06)] p-6 md:p-7">
        ...
        <div className="lg:col-span-7 order-2 space-y-3">
            ...
            {stepInfo.map((s, i) => (
              <button key={s.key} onClick={() => setOpenStep(s.key)}
                ...
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${
                  openStep === s.key ? 'bg-brand-600 text-white' : i < stepInfo.indexOf(stepInfo.find(x => x.key === openStep)!) ? 'bg-green-100 text-green-700' : 'bg-graphite-100 text-graphite-400'
                }`}>{i + 1}</span>
                <span className={`hidden md:inline ${openStep === s.key ? 'text-brand-600' : 'text-graphite-400'}`}>{s.key}</span>
                {i < stepInfo.length - 1 && <ChevronRight className="w-3 h-3 text-graphite-300" />}
              </button>
            ))}
          </div>
          ...
          {stepInfo.map((step, idx) => {
            ...
            return (
              <div key={step.key} className="bg-white rounded-2xl border border-graphite-100/80 shadow-[0_4px_16px_rgba(0,0,0,0.03)] overflow-hidden">
                <button
                  onClick={() => setOpenStep(step.key)}
                  className="w-full flex items-center gap-4 p-4 md:p-5 text-left cursor-pointer transition-colors hover:bg-graphite-50/50"
                >
                  <span className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-[11px] font-bold transition-colors ${
                    isOpen ? 'bg-brand-600 text-white' : 'bg-graphite-100 text-graphite-500'
                  }`}>{idx + 1}</span>
                  <StepIcon className={`w-4 h-4 shrink-0 transition-colors ${isOpen ? 'text-brand-600' : 'text-graphite-400'}`} />
                  <div className="flex-1 min-w-0">
                    <span className={`font-bold text-sm block transition-colors ${isOpen ? 'text-ink-900' : 'text-graphite-600'}`}>{step.title}</span>
                    <span className="text-[11px] text-graphite-400 block mt-0.5">{isOpen ? step.helper : getStepSummary(step.key)}</span>
                  </div>
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full transition-colors ${isOpen ? 'bg-brand-50 text-brand-600' : 'bg-graphite-100 text-graphite-400'}`}>
                    {isOpen ? 'Open' : 'Edit'}
                  </span>
                  <ChevronDown className={`w-4 h-4 text-graphite-400 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
                </button>
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div className="px-4 md:px-5 pb-5 md:pb-6 pt-1 border-t border-graphite-50">
          ...
          <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-graphite-100 p-4 z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${
                  build.comfort.length > 0 && build.support.length > 0 ? 'bg-ink-900 text-white shadow-md cursor-pointer' : 'bg-graphite-200 text-graphite-400 cursor-not-allowed'
                }`}
              >
                <ShoppingCart className="w-4 h-4" />
              ...
              <button onClick={handleWhatsApp}
                className="py-3 px-4 rounded-xl border-2 border-eco-500/20 text-eco-600 hover:bg-eco-50 font-semibold text-xs cursor-pointer flex items-center gap-1.5 transition-colors">
                <MessageSquare className="w-4 h-4" />
