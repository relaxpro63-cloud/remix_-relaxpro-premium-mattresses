...
import { ShoppingCart, MessageSquare, Facebook, Instagram, Youtube } from 'lucide-react';
...
scrolled
? 'bg-white/92 backdrop-blur-xl border-b border-brand-200 shadow-sm'
: 'bg-brand-50/82 backdrop-blur-md border-b border-brand-200/70'
}`}
>
{/* Top Banner */}
<div className="bg-primary text-white text-[10px] md:text-[11px] py-2 px-3 md:px-4 text-center font-accent tracking-wider flex-col md:flex-row flex items-center justify-center gap-1 md:gap-4">
...
<span className="font-semibold text-white/80">Phone 8686624494, 9642024494</span>
<span className="hidden md:inline-block text-white/30">•</span>
<span className="text-white/60 font-semibold">Hyderabad • Rajahmundry • Bangalore</span>
</div>
  ...
  <nav className="hidden lg:flex items-center gap-7" role="navigation" aria-label="Main navigation">
    {navItems.map((item) => (
      <Link
        key={item.path}
        to={item.path}
        className={`relative text-xs font-semibold uppercase tracking-wider font-accent py-1 transition-colors cursor-pointer ${
          isActive(item.path) ? 'text-primary' : 'text-neutral-dark/60 hover:text-primary'
        }`}
      >
        {item.label}
        {isActive(item.path) && (
  ...
  <div className="px-6 py-6 border-t border-brand-200 space-y-4">
    ...
    <div className="flex items-center justify-center gap-5 pt-1">
      <a
        ...
        className="text-neutral-dark/40 hover:text-primary transition-colors"
        title="Facebook"
      >
