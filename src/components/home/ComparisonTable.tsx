    ...
    <section className="py-12 md:py-16 px-4 md:px-8 bg-sky-100/20 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        <div className="text-center mb-10 md:mb-14">
          ...
          <p className="text-graphite-500 font-body text-sm md:text-base">
        ...
        <div className="rounded-2xl overflow-hidden border border-brand-200/50 shadow-sm font-body">
          <div className="grid grid-cols-2 bg-eco-50">
            <div className="py-4 md:py-6 text-center border-r border-brand-200/50">
              <h3 className="text-base md:text-lg font-bold text-ink-900 font-heading">RelaxPro</h3>
            </div>
            <div className="py-4 md:py-6 text-center">
              <h3 className="text-base md:text-lg font-bold text-graphite-600 font-heading">Other Brands</h3>
            </div>
          </div>
            ...
            {data.items.map((row: any, idx: number) => (
              ...
              >
                <div className="p-4 md:p-6 flex flex-col md:flex-row items-center justify-start gap-3 md:gap-4 border-r border-brand-200/30 text-center md:text-left">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-eco-50 flex items-center justify-center shrink-0">
                    <div className="scale-75 md:scale-100 flex items-center justify-center">{iconMap[row.icon] || iconMap['check']}</div>
                  </div>
                  <span className="font-semibold text-ink-900 text-sm md:text-base leading-tight">{row.relaxProTitle}</span>
                </div>
                
                <div className="p-4 md:p-6 flex flex-col justify-center gap-1">
                  <div className="flex flex-col md:flex-row items-center justify-start gap-2 md:gap-3 text-graphite-600 text-center md:text-left">
                  ...
                  {row.otherSubtext && (
                    <p className="text-[10px] md:text-xs text-graphite-400 text-center md:text-left md:pl-8 leading-tight mt-1 md:mt-0">
                      {row.otherSubtext}
                    </p>
                  )}
                </div>
