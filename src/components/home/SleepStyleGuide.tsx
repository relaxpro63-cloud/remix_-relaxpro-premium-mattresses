...
import { getSleepStyles, imageUrl } from '../../lib/queries';

const defaultStyles = [
  { title: 'Side Sleeper', desc: 'Need pressure relief on shoulders & hips. Try our plush organic latex or soft-medium hybrids.', slug: 'nirvana', linkText: 'View luxury models' },
  { title: 'Back & Orthopedic', desc: 'Require spine alignment & firm support. Our medium-firm orthopedic builds are doctor-recommended.', slug: 'amrita', linkText: 'View premium models' },
  { title: 'Stomach Sleeper', desc: 'Need firm even surface to prevent hip sinking. Our firm and ultra-firm options keep you aligned.', slug: 'ananda', linkText: 'View comfort models' },
];
...
export default function SleepStyleGuide() {
  ...
  useEffect(() => {
    getSleepStyles().then(d => { if (d?.length > 0) setStyles(d); }).catch(() => {});
  }, []);
